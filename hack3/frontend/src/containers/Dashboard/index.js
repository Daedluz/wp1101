...
import {
  ...
  // DONE TODO 6.5 Uncomment the following line
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // DONE TODO 6.5 subscription logic
    subscribeToMore({
      document: TASK_DELETED_SUBSCRIPTION,
      updateQuery: (prev, { subscriptionData }) =>
      {
        if (!subscriptionData.data) return prev;
        const { data: {taskDeleted} } = subscriptionData;
        return {
  ...
  const handleUpdateTask = (id, status) => {
    ...
    // DONE TODO 3 Use `updateTask` and pass the correct variables
    updateTask({
      variables: {
        id: id,
        status: status,
